...
L  0  260319  5073  001
│  │  │       │     │
│  │  │       │     └── 일련번호 (3자리): 해당 업체 그날 작업 순서
│  │  │       └──────── 영업자코드 (4자리): 포장처리업 고유 코드
│  │  └──────────────── 작업날짜 (6자리): YYMMDD
│  └─────────────────── 축종구분 (1자리): 0=소, 1=돼지
└────────────────────── Lot 표시 (1자리): 고정 'L'

예시:
  소:     L0260319507300 1  (L + 0(소) + 260319 + 5073 + 001)
  돼지:   L1260319507300 1  (L + 1(돼지) + 260319 + 5073 + 001)
```

### 5-2. 묶음번호 생성 조건

- 이력번호가 **다른** 여러 개체를 하나로 포장할 경우 사용
- 한 묶음에 **최대 30개** 이력번호까지 포함 가능
- 영업자코드는 **mtrace.go.kr 가입 시 부여**받는 고유 코드

### 5-3. 시스템 자동 생성 로직

```python
def generate_lot_no(species, business_code, production_date, sequence):
    """법정 묶음번호 15자리 생성.

    Args:
        species: 'cattle'(소) 또는 'pig'(돼지)
        business_code: 영업자코드 4자리 (mtrace 부여)
        production_date: 작업일 (YYMMDD)
        sequence: 당일 일련번호 (1~999)

    Returns:
        str: 15자리 묶음번호 (예: L02603195073001)
    """
    species_code = '0' if species == 'cattle' else '1'
    date_str = production_date.strftime('%y%m%d')
    seq_str = str(sequence).zfill(3)
    return f'L{species_code}{date_str}{business_code}{seq_str}'
```
...
|------|----------|
| 도축장 연접 식육포장처리업 | **필수** |
| 종업원 5인 이상 식육포장처리업 | **필수** |
| 그 외 소규모 | 자체 장부 기록 (전산신고 희망 시 가능) |
...
① 매입신고 (입고)
   → 원료육 받은 날로부터 5일 이내
   → 신고항목: 이력번호, 부위, 중량, 매입처(도축장/유통업체), 매입일

② 포장처리신고 (묶음번호 생성)
   → 포장처리 후 5일 이내
   → 신고항목: 생성된 묶음번호, 투입 이력번호들, 부위, 중량, 작업일

③ 판매반출신고 (출고)
   → 판매/반출 후 5일 이내
   → 신고항목: 묶음번호(또는 이력번호), 판매처, 부위, 중량, 판매일
```
...
| 방법 | 설명 |
|------|------|
| **mtrace.go.kr 웹** | 포장처리업 로그인 → 수동 입력 |
| **축산물이력제 신고 앱** | 모바일 앱 (QR/바코드 스캔) |
| **엑셀 일괄업로드** | mtrace 양식 엑셀 작성 → 업로드 |
| **API 연동** | ⚠️ B2B 별도 협의 (공개 API 미확인) |

### 6-4. 우리 시스템의 자동화 전략

```
방법 1 (API 가능 시):
  시스템 → mtrace API 직접 호출 → 자동 신고 완료

방법 2 (API 불가 시 — 현실적 대안):
  시스템 → mtrace 양식 엑셀 자동 생성 → 다운로드
  사용자 → mtrace.go.kr에서 엑셀 업로드 (클릭 2번)

  매입신고 엑셀 자동 생성:
    [이력번호] [부위] [중량] [매입처] [매입일]

  포장처리신고 엑셀 자동 생성:
    [묶음번호] [투입이력번호1] [투입이력번호2] ... [부위] [중량] [작업일]
