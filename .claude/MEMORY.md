...
## 필수 규칙 (모든 PC/AI 공통)
1. **코드 수정 전 이 메모리 파일을 반드시 읽을 것** — 추측으로 수정 금지
2. **커밋할 때마다 이 메모리 파일도 함께 업데이트할 것** — 특히 DB/테이블/환경변수 변경 시
3. **DB 테이블명 변경 전 Supabase 대시보드에서 실제 스키마 확인할 것**
4. **에러 발생 시 코드 수정 전 DB 스키마부터 확인할 것**
5. 교훈: 메모리 없이 AI가 추측으로 수정하면 3PC 간 되돌리기 충돌 발생 (2026-03-25 사고)
