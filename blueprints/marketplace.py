...
def _sanitize_receiver_name(name, fallback_name=''):
    """수취인명 검증 — 공유 모듈 래퍼."""
    from services.api_order_converter import sanitize_receiver_name
    return sanitize_receiver_name(name, fallback_name)


def _api_orders_to_excel_df(orders, channel):
    """API raw_data → 채널별 엑셀 DataFrame — 공유 모듈 래퍼."""
    from services.api_order_converter import api_orders_to_excel_df
    return api_orders_to_excel_df(orders, channel)
